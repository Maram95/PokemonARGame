...
class ViewController: UIViewController, ARSCNViewDelegate, SCNPhysicsContactDelegate {
    ...
    //creates ball  node and 'fires' it
    func fireMissile(type : String){
        ...
        switch type {
        case "PokeBall":
            nodeDirection  = SCNVector3(direction.x*4,direction.y*4,direction.z*4)//sound change
            node.physicsBody?.applyForce(nodeDirection, at: SCNVector3(0.1,0,0), asImpulse: true)
            playSound(sound: "pokeball opening sound FX", format: "mp3")
    ...
    func createMissile(type : String)->SCNNode{
        ...
        switch type {
        case "PokeBall":
            let scene = SCNScene(named: "art.scnassets/PokeBall_ColladaMax.DAE")
            node = (scene?.rootNode.childNode(withName: "PokeBall", recursively: false)!)!
    ...
    func addTargetNodes(){
        for index in 1...100 {
            ...
            if (index > 9) && (index % 10 == 0) {
                let scene = SCNScene(named: "art.scnassets/Pidgey.DAE")
                node = (scene?.rootNode.childNode(withName: "Pidgey", recursively: false)!)!
                node.scale = SCNVector3(0.03,0.03,0.03)
                node.name = "Pidgey"
            }else{
                let scene = SCNScene(named: "art.scnassets/Jigglypuff.DAE")
                node = (scene?.rootNode.childNode(withName: "Jigglypuff", recursively: false)!)!
                //node.scale = SCNVector3(0.002,0.002,0.002)
                node.scale = SCNVector3(0.02,0.02,0.02)
                node.name = "Jigglypuff"
            }
    ...
    func physicsWorld(_ world: SCNPhysicsWorld, didBegin contact: SCNPhysicsContact) {
        ...
        if contact.nodeA.physicsBody?.categoryBitMask == CollisionCategory.targetCategory.rawValue
            ...
            if (contact.nodeA.name! == "Pidgey" || contact.nodeB.name! == "Pidgey") {
                score+=5
            }else{
                score+=1
            }
    ...
    func playBackgroundMusic(){
        ...
        let audioSource = SCNAudioSource(fileNamed: "themeMusic.mp3")!
